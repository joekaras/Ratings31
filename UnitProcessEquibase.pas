unit UnitProcessEquibase;

interface

implementation
procedure TMainForm.ProcessEquibaseStats(Sender: TObject; sStatType: string; sYear: string);
var
   iEndPos: Integer;
   iStartPos: Integer;
   iAttributes: Integer;
...
begin
   ...
   hLog.LogFileName := LOG_PATH + EQUIBASE_LOG_FILENAME;
   hLog.AddToLog('ProcessEquibaseStats', msevOperation);

   try
      OpenTableExclusive(dm.tblTrainer);
      OpenTableExclusive(dm.tblJockey);
      OpenTableExclusive(dm.tblOwner);
      ...
      CloseTableExclusive(dm.tblTrainer);
      CloseTableExclusive(dm.tblJockey);
      CloseTableExclusive(dm.tblOwner);
   end;
...
procedure TMainForm.ParseEquibaseStats(sFileName: string; sSaveFileName: string; sStatType: string; sYear: string; tbl: TDBISAMTable);
var
   sIn: string;
   sOut: string;
