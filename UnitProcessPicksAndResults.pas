unit UnitProcessPicksAndResults;
...
procedure ProcessPicksAndResults(tblRH: TDBISAMTable; bUseOverride: boolean);
...
var
   ...
   iSuperSelections1st: TNums;
   iSuperSelections2nd: TNums;
   iSuperSelections3rd: TNums;
   iSuperSelections4th: TNums;
   ...
   try
      ...
      try
         ...
         if (bUseOverride) then begin
            ...
            tblRH.SetRangeStart;
            tblRH.FieldByName('RangeRaceDate').AsDateTime := gedtOverrideDate;
         ...
         while not tblRH.Eof do begin
            ...
            // 1W2-3W2-3-4-5W2-3-4-5-6-7)
            // 2-3-4-5W1W2-3W2-3-4-5-6-7)
            ...
            if (tblRH.FieldByName('SuperfectaPayout').AsFloat > 0) then begin
               ...
               try
                  ...
                  for iIdx := 1 to 20 do begin
                     Exclude(iSuperSelections1st, iIdx);
                     Exclude(iSuperSelections2nd, iIdx);
                     Exclude(iSuperSelections3rd, iIdx);
                     Exclude(iSuperSelections4th, iIdx);
                  end;

                  sSuperPicks := tblRH.FieldByName('Super').AsString;
                  sSuperPicks := FastReplace(sSuperPicks, 'W', ',', True);
                  Split(CleanEntryNbrs(sSuperPicks), ',', lstLegs);

                  if (lstLegs.Count > 3) then begin
                     sSuper1st := StringListTrim(lstLegs[0]);
                     sSuper2nd := StringListTrim(lstLegs[1]);
                     sSuper3rd := StringListTrim(lstLegs[2]);
                     sSuper4th := StringListTrim(lstLegs[3]);
                     //end else begin
                     //   hLog.AddToLog('lstLegs Count', msevException)
                  end;
                  FreeAndNil(lstLegs);

                  Split(sSuper1st, '-', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     Include(iSuperSelections1st, atoi(lstSFC[iIdx]));
                  end;
                  FreeAndNil(lstSFC);

                  Split(sSuper2nd, '-', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     Include(iSuperSelections2nd, atoi(lstSFC[iIdx]));
                  end;
                  FreeAndNil(lstSFC);

                  Split(sSuper3rd, '-', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     Include(iSuperSelections3rd, atoi(lstSFC[iIdx]));
                  end;
                  FreeAndNil(lstSFC);

                  Split(sSuper4th, '-', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     Include(iSuperSelections4th, atoi(lstSFC[iIdx]));
                  end;
                  FreeAndNil(lstSFC);
                  ...
                  if (lstResults.Count > 3) then begin
                     ...
                     hLog.AddToLog('lstResults Count', msevException)
                  end;
                  FreeAndNil(lstResults);

                  // Key
                  Split(sResultsWinHorse, '/', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     if atoi(lstSFC[iIdx]) in iSuperSelections1st then begin
                  ...
                  //
                  Split(sResultsPlcHorse, '/', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     if atoi(lstSFC[iIdx]) in iSuperSelections2nd then begin
                  ...
                  //
                  Split(sResultsShoHorse, '/', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     if atoi(lstSFC[iIdx]) in iSuperSelections3rd then begin
                  ...
                  //
                  Split(sResults4thHorse, '/', lstSFC);
                  for iIdx := 0 to lstSFC.Count - 1 do begin
                     if atoi(lstSFC[iIdx]) in iSuperSelections4th then begin
